...
export function DegreeList() {
  ...
  const loadDegrees = async () => {
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to load degrees"
      );
    }
  };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to create degree level"
      );
    } finally {
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to update degree level"
      );
    } finally {
      setFormLoading(false);
    }
  ...
  const handleDelete = async (degree: DegreeLevel) => {
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to delete degree level"
      );
    }
  };
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">No degree levels found</p>
              {canCreateEdit && (
                <Button
                  className="mt-4"
              ...
              <TableBody>
                {degrees.map((degree) => (
                  <TableRow key={degree.degree_level_id}>
                    <TableCell>
                      <Badge variant="outline">{degree.degree_level_id}</Badge>
                    </TableCell>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    ...
                                    permanently delete the degree level &ldquo;
                                    {degree.level_name}&rdquo; and may affect
          ...
          <form onSubmit={handleCreate}>
            ...
            <DialogFooter>
              <Button type="submit" disabled={formLoading}>
                {formLoading ? "Creating..." : "Create Degree Level"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      >
        <DialogContent className="sm:max-w-[525px]">
          <form onSubmit={handleEdit}>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="edit_level_name" className="text-right">
                  Level Name
                </Label>
                <Input
                  id="edit_level_name"
            ...
            <DialogFooter>
              <Button type="submit" disabled={formLoading}>
                {formLoading ? "Updating..." : "Update Degree Level"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
