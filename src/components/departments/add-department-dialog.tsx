...
export function AddDepartmentDialog({
	...
	async function onSubmit(values: CreateDepartmentRequest) {
		...
		setIsLoading(true);
		try {
			await DepartmentService.createDepartment(token, values);
		...
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent className="sm:max-w-[425px]">
				<DialogHeader>
					...
					<DialogDescription>
						Create a new department. This will also create an HOD
						account with the provided email.
					</DialogDescription>
				</DialogHeader>
				<Form {...form}>
					<form
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormDescription>
										Email address for the Head of Department
										(HOD account will be created)
									</FormDescription>
									<FormMessage />
								</FormItem>
							)}
